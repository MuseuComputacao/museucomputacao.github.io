...
Nesse post aprendemos como customizar o tema padrão no framework jekyll. Você pode encontrar o repositório com esse projeto [aqui](https://github.com/ThiagoBarcellos/Jekyll-change-theme-example) para poder baixar e usar como quiser. Para ficar ligado em mais posts como esse considere assinar nosso [feed]({{site.url}}/feed.xml) ou acessar a nossa [seção de posts do jekyll]({{site.url}}/posts/#jekyll).